...
The core ``embed``/``extract`` functions work on Pillow ``Image`` objects, so
the file-based :func:`encode`/:func:`decode` and the in-memory web server
...
def embed(img: "Image.Image", message: str) -> "Image.Image":
    ...
    payload = MAGIC + len(raw).to_bytes(4, "big") + raw
...
def encode(input_path: ImageSource, output_path: PathLike, message: str) -> Path:
    ...
    with Image.open(input_path) as img:
        result = embed(img, message)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    result.save(output_path)
    return output_path


def decode(image_path: ImageSource) -> str:
